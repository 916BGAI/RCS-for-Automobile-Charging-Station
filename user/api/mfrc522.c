...
void MFRC_WriteReg(u8 addr, u8 data) {
  ...
  CS_522 = 0;                    // NSS拉低
  Soft_SPI_Write(AddrByte);      // 写地址字节
  Soft_SPI_Write(data);          // 写数据
  CS_522 = 1;                    // NSS拉高
}
...
void MFRC522_Add_IC(void) {
  while (1) {
	if (Card_GetNumber(PICC_REQALL, Get_ID_Card) != 0) // 如果没有获得正确的卡号
	{
	  Draw_Text_8_16_Str(50, 80, WHITE, BLUE, "充电请刷卡.......");
